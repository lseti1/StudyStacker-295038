...
export default function DeckCards({ deckId, onCardClick, highlight }: DeckCardsProps) {
                            ...
                            {flashcards.map((card) => (
                                <div key={card.id} onClick={() => onCardClick(card)} className="relative bg-white h-60 w-[32%] border box-shadow-light flex items-center justify-center text-3xl hover:bg-gray-100 hover:cursor-pointer select-none duration-500">
                                    ...
                                    {highlight && <div className="absolute flex flex-row top-2 right-4 gap-3 justify-center items-center text-gray-400">
                                            <button className="text-lg font-bold hover:text-black">X</button>
                                            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 hover:text-black hover:cursor-pointer duration-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
