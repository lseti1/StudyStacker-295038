...
export default function DecksSidebar({ decks, onSelectDeck} : DecksSidebarProps) {
    return (
        <div>
            {decks.length === 0 ? (
                <p>No Decks Found</p>
                ...
                <ul>
                    {decks.map(deck => (
                        <div className='flex justify-between items-center bg-black h-[8%] px-5 py-2 border-y border-gray-500 hover:cursor-pointer hover:bg-gray-900 duration-500' key={deck.id}>
                            <h1 className='text-2xl tracking-tighter'>{deck.name}</h1>
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-gray-600 hover:text-gray-100 duration-500" onClick={() => onSelectDeck(deck)} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
